...
import { Bot, ChevronLeft, ChevronRight, FileText, Image, Menu, Mic, Moon, Paperclip, Plus, Send, Settings, Sparkles, Speaker, Square, Sun, Users, Video, VolumeX, X } from "lucide-react";
import "./styles.css";
...
function App() {
  ...
  useEffect(() => { const saved = localStorage.getItem(`erpel.messages.${mode}`); setMessages(saved ? JSON.parse(saved) : []); setConversationId(undefined); }, [mode]);
  useEffect(() => { if (configured) localStorage.setItem(`erpel.messages.${mode}`, JSON.stringify(messages.slice(-100))); bottomRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, mode, configured]);
  ...
  const send = async (text = draft) => {
    ...
    try { const job = await api("/chat", { method: "POST", body: JSON.stringify({ message: clean || `Analysiere ${sentAttachment?.name}`, mode, model: selectedModel || undefined, conversationId, attachment: sentAttachment }) }); setCurrentJobId(job.id); completeResult(await pollJob(job.id), id); }
  ...
  const pickFile = async (file?: File) => {
    ...
    setAttachment({ name: file.name, mimeType: file.type || "application/octet-stream", base64: dataUrl.split(",")[1] ?? "", preview: file.type.startsWith("image/") ? dataUrl : undefined });
  };
  ...
  const mainAvatar = agentPetId || mainBot?.avatar;
  ...
  return <div className="app-shell">
    {mobileSidebar && <div className="sidebar-scrim" onClick={() => setMobileSidebar(false)}/>}<aside className={`sidebar ${sidebar ? "" : "collapsed"} ${mobileSidebar ? "mobile-open" : ""}`}>
      ...
      {mode === "team" && <section className="bot-overview"><div className="section-title"><span>Agenten & Bots</span><button onClick={loadBots}>↻</button></div>{botsLoading ? <small>Lade Team …</small> : bots.map(bot => <div className="bot-row" key={bot.slug}><AgentAvatar avatar={bot.avatar} name={bot.name} size={30}/><div><strong>{bot.name}</strong><small>{bot.description || bot.slug}</small></div></div>)}</section>}
