...
function parsePart(part: string, min: number, max: number): Set<number> {
  ...
  for (const tokenRaw of trimmed.split(",")) {
    ...
    if (token.includes("/")) {
      ...
      if (!Number.isFinite(step) || step <= 0) throw new Error(`Invalid cron step '${token}'`);
      ...
      if (base === "*") {
        ...
        if (!Number.isFinite(start) || !Number.isFinite(end)) throw new Error(`Invalid cron range '${token}'`);
        for (let i = start; i <= end; i += step) {
          if (i >= min && i <= max) result.add(i);
        }
      } else {
        const start = Number.parseInt(base, 10);
        if (!Number.isFinite(start)) throw new Error(`Invalid cron value '${token}'`);
    ...
    if (!Number.isFinite(value)) throw new Error(`Invalid cron value '${token}'`);
    if (value < min || value > max) throw new Error(`Cron value '${value}' out of range ${min}-${max}`);
    result.add(value);
  }

  if (result.size === 0) throw new Error("Cron field resolved to empty set");
  return result;
}
...
function matches(date: Date, fieldSet: CronFieldSet): boolean {
