...
export class SileroSpeechToTextProvider extends BaseSpeechToTextProvider {
  ...
  async transcribe(audioBuffer: Buffer, options?: { language?: string }): Promise<string> {
    ...
    throw new Error(
      "Silero speech-to-text requires either: (1) Ollama with speech model, or (2) local speech-to-text service"
    );
  }

  private async transcribeViaOllama(audioBuffer: Buffer, language?: string): Promise<string> {
    ...
    try {
      ...
      throw new Error(
        `Silero transcription via Ollama failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
