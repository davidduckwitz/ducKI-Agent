...
export class LocalCommandSpeechToTextProvider extends BaseSpeechToTextProvider {
  readonly name = "local";
  private readonly localOptions: LocalSpeechToTextProviderOptions;
  ...
  async transcribe(audioBuffer: Buffer, options?: { language?: string }): Promise<string> {
    ...
    if (!command) {
      throw new Error(
        "Local STT command not configured. Set LOCAL_STT_COMMAND (for example whisper-cli) and LOCAL_STT_ARGS (for example '-f {input}')"
      );
    }

    const argsTemplate =
      this.localOptions.args ??
      parseArgsTemplate(
        process.env["LOCAL_STT_ARGS"] ??
    ...
    try {
      ...
      if (run.exitCode !== 0) {
        throw new Error(
          `Local STT command failed with exit code ${run.exitCode}${run.stderr.trim() ? `: ${run.stderr.trim()}` : ""}`
        );
      }
