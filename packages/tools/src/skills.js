...
function frontmatterScript(content) {
    if (!content.startsWith("---"))
        return undefined;
    const end = content.indexOf("\n---", 3);
    if (end < 0)
...
function extractInlineScript(content) {
...
function sanitizeRuntimeValue(value, depth = 0) {
    if (depth > 6) {
...
function runScriptInSandbox(script, runtime) {
    const logs = [];
    const logger = (...args) => {
        logs.push(args
            .map((arg) => {
        ...
        try {
            ...
            switch (action) {
                ...
                case "rename": {
                    ...
                    if (!sourceName)
                        return fail("oldSkillName is required");
                    if (!newSkillName)
                        return fail("newSkillName is required");
                    const sourceDir = skillDir(sourceName);
                    const targetDir = skillDir(newSkillName);
                    if (!existsSync(sourceDir))
                        return fail(`Skill not found: ${sourceName}`);
                    if (existsSync(targetDir))
                        return fail(`Skill already exists: ${newSkillName}`);
                ...
                case "remove_file": {
                    ...
                    if (statSync(absolutePath).isDirectory())
                        return fail("remove_file expects a file, not a directory");
                ...
                case "execute": {
                    ...
                    if (directScriptFile) {
                        ...
                        if (!absolute.startsWith(dir))
                            return fail("script_file escapes the skill directory");
                        if (!existsSync(absolute))
                            return fail(`Script file not found: ${rel}`);
                        source = rel;
                        ...
                        if (configuredScript) {
                            ...
                            const absolute = resolve(dir, rel);
                            if (!absolute.startsWith(dir))
                                return fail("Configured script escapes the skill directory");
                            if (!existsSync(absolute))
                                return fail(`Configured script not found: ${rel}`);
                            source = rel;
                            script = readFileSync(absolute, "utf8");
                        }
                        else {
