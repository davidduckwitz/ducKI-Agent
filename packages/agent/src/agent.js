...
export class Agent {
    ...
    autoSkillSelectionAttempts = 0;
    autoSkillSelections = 0;
    ...
    scoreSkillMatch(input, skill) {
        ...
        if (inputTokens.size === 0 || skillTokens.size === 0)
            return 0;
        let intersection = 0;
        ...
        if (skill.slug === "workflow-orchestrator" &&
        ...
        if (/(wiki|wissen|knowledge|dokumentation|docs|nachschlagen|recherche|quelle|sources|llm-wiki)/.test(normalizedInput) &&
            /(llm-wiki|knowledge-base|wiki)/.test(skill.slug)) {
            score += 0.5;
        }
        if (/(welcher\s*tag|welchen\s*tag|wochentag|heute|datum|uhrzeit|date|time|day\s+is\s+it|what\s+day\s+is\s+it)/.test(normalizedInput) &&
            /(datum-uhrzeit-tag|datum-uhrzeit|date-time)/.test(skill.slug)) {
            score += 0.55;
        }
        return Math.min(1, score);
    }
    isDateTimeIntent(input) {
        const normalizedInput = input.toLowerCase();
    ...
    rankSkillMatches(input, installed, alreadySelectedSlugs) {
        return installed
    ...
    selectAutoSkill(input, installed, alreadySelectedSlugs, controls) {
        if (!controls.enableAutoSkillSelection)
            return { scored: [], reason: "disabled" };
        if (input.trim().length < controls.autoSkillMinInputLength)
            return { scored: [], reason: "input_too_short" };
        const scored = this.rankSkillMatches(input, installed, alreadySelectedSlugs);
        const best = scored[0];
        const second = scored[1];
        if (!best)
            return { scored, reason: "no_candidates" };
        if (best.score < controls.autoSkillScoreThreshold)
            return { scored, reason: "below_threshold" };
        if (best.overlap < controls.autoSkillMinOverlap)
            return { scored, reason: "overlap_too_low" };
    ...
    extractRequestedSkillSlugs(text) {
    ...
    resolveToolNameAndInput(toolName, input) {
        ...
        if (normalized === "task" && normalizedInput["action"] === undefined) {
            ...
            else if (normalizedInput["status"] !== undefined ||
        ...
        if (normalized === "task" && normalizedInput["action"] !== undefined) {
            ...
            if (normalizedAction === "get") {
                ...
                const hasUpdateFields = ["status", "priority", "result", "subtasks", "title", "description", "projectId", "project_id"].some((key) => normalizedInput[key] !== undefined);
                if (hasUpdateFields) {
                    if (status === "completed") {
    ...
    preflightToolInput(toolName, input, controls) {
        ...
        if (normalizedName === "filesystem") {
            ...
            if (!action)
                return { ok: false, error: "filesystem: action is required" };
        ...
        if (normalizedName === "http") {
            const action = String(normalizedInput["action"] ?? "").toLowerCase();
            if (!action)
                return { ok: false, error: "http: action is required" };
        ...
        if (normalizedName === "project") {
            const action = String(normalizedInput["action"] ?? "").toLowerCase();
            if (!action)
                return { ok: false, error: "project: action is required" };
        ...
        if (normalizedName === "task") {
            ...
            if (!action)
                return { ok: false, error: "task: action is required" };
        ...
        if (normalizedName === "skill_manage") {
            ...
            if (!action)
                return { ok: false, error: "skill_manage: action is required" };
        ...
        if (normalizedName === "memory") {
            ...
            normalizedInput["action"] = action;
            if (!action)
                return { ok: false, error: "memory: action is required" };
            ...
            if (action === "batch") {
                if (!Array.isArray(normalizedInput["operations"]) || normalizedInput["operations"].length === 0) {
        ...
        if (normalizedName === "workflow") {
            ...
            normalizedInput["action"] = action;
            if (!action)
                return { ok: false, error: "workflow: action is required" };
        ...
        if (normalizedName === "history") {
            const action = String(normalizedInput["action"] ?? "").toLowerCase();
            if (!action)
                return { ok: false, error: "history: action is required" };
        ...
        if (normalizedName === "gateway") {
            const action = String(normalizedInput["action"] ?? "").toLowerCase();
            if (!action)
                return { ok: false, error: "gateway: action is required" };
    ...
    parseHermesArgs(rawArgs) {
        const source = rawArgs;
        const out = {};
        let i = 0;
        const decodeTokenQuotes = (value) => value.replaceAll('<|"|>', '"').replaceAll("<|'|>", "'");
        const skipWs = () => {
        ...
        const readDelimitedValue = (delimiter) => {
        ...
        const readValue = () => {
            ...
            if (!Number.isNaN(asNum) && raw.length > 0)
                return asNum;
    ...
    extractToolCall(response) {
        ...
        const bracketBody = (() => {
            ...
            if (bracketBodyMatch?.[1])
                return bracketBodyMatch[1].trim();
    ...
    buildToolCallSignature(toolName, input) {
        const stable = JSON.stringify(input, Object.keys(input).sort());
        return `${toolName}:${stable}`;
    }
    deriveToolRecoveryHint(toolName, toolInput, error) {
    ...
    parseBooleanSetting(raw, fallback) {
        if (raw === undefined)
            return fallback;
    ...
    parseNumberSetting(raw, fallback, min, max) {
        if (raw === undefined)
            return fallback;
    ...
    parseFloatSetting(raw, fallback, min, max) {
        if (raw === undefined)
            return fallback;
        const parsed = Number(raw);
        if (!Number.isFinite(parsed))
    ...
    parseEnabledSkillSlugs(rawValue) {
    ...
    parseSkillBehavior(raw, fallback) {
        if (!raw)
            return fallback;
        const normalized = raw.trim().toLowerCase();
        if (normalized === "automatic" || normalized === "auto")
            return "automatic";
        if (normalized === "active" || normalized === "all_activated")
            return "active";
        return fallback;
    }
    async loadRuntimeControls() {
        const defaults = {
    ...
    async runLoop(userInput, toolsUsed, iterations, controls, options) {
        ...
        const rememberSuccessfulTool = async (toolName, toolInput, toolResult) => {
            if (!controls.enableAutoMemory)
                return;
            if (!toolResult.success)
                return;
            try {
                const decision = await this.memory.rememberFromSuccessfulTool(toolName, toolInput, toolResult.data, this.conversation.id);
                if (decision.stored) {
        ...
        let activeSkillManifests = [...prioritizedRequestedSkillManifests];
        ...
        const relatedSkillManifests = this.expandRelatedSkills(activeSkillManifests, installedSkillManifests, enabledAllowlist).filter((skill) => !activeSkillManifests.some((current) => current.slug === skill.slug));
        ...
        if (activeSkills.length > 0) {
            emit("reasoning", `Skills geladen: ${activeSkills.map((s) => s.slug).join(", ")}`, { skills: activeSkills.map((s) => ({ slug: s.slug, name: s.name })) });
        }
        if (relatedSkillManifests.length > 0) {
        ...
        const compactBaseSystemPrompt = this.systemPrompt +
            installedSkillsContext +
            ...
            memoryContext +
            "\n\n## Task Rules\n- Create a project before creating project-specific tasks when the work should be tracked long-term.\n- Mark a task running before execution and completed or failed when finished.\n- Persist results in the database so the UI can show progress.\n- Use tools whenever state must change.\n- Never repeat the exact same tool call more than once without changing input or strategy.\n- If a tool fails, correct parameters based on the error before retrying.\n- If /workflow-orchestrator is loaded, first drive the workflow lifecycle (list/get/create/update/run/resume) before unrelated tools.\n- For stable user or workflow facts, use memory tool actions to recall or curate durable memory.\n- Treat only explicit requests to send, post, answer, or reply on Discord as outbound gateway operations, not normal chat replies.\n- For Discord/gateway outbound send requests, always run gateway action=list_configs before gateway action=send in the same run.\n- If the Discord target is unclear, ask for the target channel instead of guessing.\n- Never guess localhost/default Discord endpoints if gateway configs exist; rely on gateway tool diagnostics and configured transports.";
        const minimalBaseSystemPrompt = this.systemPrompt +
            toolContext +
            (planContext ? `\n\n## Working Plan\n${JSON.stringify(planContext, null, 2)}` : "") +
            memoryContext +
            "\n\n## Task Rules\n- Create a project before creating project-specific tasks when the work should be tracked long-term.\n- Mark a task running before execution and completed or failed when finished.\n- Persist results in the database so the UI can show progress.\n- Use tools whenever state must change.\n- Never repeat the exact same tool call more than once without changing input or strategy.\n- If a tool fails, correct parameters based on the error before retrying.\n- If /workflow-orchestrator is loaded, first drive the workflow lifecycle (list/get/create/update/run/resume) before unrelated tools.\n- For stable user or workflow facts, use memory tool actions to recall or curate durable memory.\n- Treat only explicit requests to send, post, answer, or reply on Discord as outbound gateway operations, not normal chat replies.\n- For Discord/gateway outbound send requests, always run gateway action=list_configs before gateway action=send in the same run.\n- If the Discord target is unclear, ask for the target channel instead of guessing.\n- Never guess localhost/default Discord endpoints if gateway configs exist; rely on gateway tool diagnostics and configured transports.";
        ...
        const sanitizeCap = (value, minimum, fallback) => {
            if (!Number.isFinite(value))
                return fallback;
            const rounded = Math.floor(value);
            return rounded >= minimum ? rounded : fallback;
        };
        const envCap = (key, fallback, minimum) => {
            const parsed = Number.parseInt(process.env[key] ?? "", 10);
            return sanitizeCap(parsed, minimum, fallback);
        };
        const withOverride = (override, fallback, minimum) => {
        ...
        while (iterations < controls.maxIterations) {
            ...
            const buildConversationWindow = (messageLimit, charLimit) => {
                ...
                for (let index = allMessages.length - 1; index >= 0; index--) {
                    ...
                    if (!message)
                        continue;
                    if (selected.length >= Math.max(1, messageLimit))
                        break;
                    ...
                    if (selected.length > 0 && nextChars > Math.max(2000, charLimit))
                        break;
                    selected.push({
                        ...message,
            ...
            const buildMessages = (mode, contextOptions) => {
                ...
                const contextMessages = buildConversationWindow(contextOptions?.messageLimit ?? maxContextMessages, contextOptions?.charLimit ?? maxContextChars);
                return [systemMessage, ...contextMessages];
            };
            const generateFromMessages = async (messages) => {
            ...
            if (hasToolCallMarker && !parsedToolCall) {
                ...
                const repairHint = {
                    role: "system",
                    content: "Your previous tool call format was malformed or incomplete. Re-emit exactly one valid tool call in one of these forms: [TOOL:name({json})] or <|tool_call>call:name({json}). Do not add prose.",
                };
            ...
            if (!parsedToolCall) {
                const reasoning = await this.reasoner.reason([...messages, assistantMessage], availableTools.map((tool) => tool.name), `User request: ${effectiveInput}`);
                ...
                else if ((reasoning.action === "respond" || reasoning.action === "ask_clarification") &&
            ...
            if (workflowOrchestratorRequested && iterations <= 2 && parsedToolCall.toolName !== "workflow") {
                const enforcedResult = {
                    success: false,
                    data: null,
                    error: "Policy violation: with /workflow-orchestrator, start by using workflow actions (list/get/create/update/run/resume) before unrelated tools.",
                };
            ...
            try {
                ...
                if (toolResult.success) {
                    ...
                    consecutiveToolFailures++;
                    const recoveryHint = this.deriveToolRecoveryHint(parsedToolCall.toolName, parsedToolCall.input, String(toolResult.error ?? ""));
                    if (recoveryHint) {
                        const hintMessage = {
                            role: "system",
        ...
        let reflectionQuality;
        let reflectionIssueSnapshot = [];
        if (controls.enableReflection && controls.reflectionMaxRetries > 0 && finalResponse.trim().length > 0) {
            for (let reflectionAttempt = 1; reflectionAttempt <= controls.reflectionMaxRetries; reflectionAttempt++) {
                const reflectionResult = await this.reflection.evaluate(effectiveInput, this.sanitizeFinalResponse(finalResponse), `toolsUsed=${toolsUsed.join(",")}; iterations=${iterations}`);
        ...
        if (controls.enableReflection && controls.reflectionMetaReview && finalResponse.trim().length > 0) {
            const metaReflection = await this.reflection.evaluate(effectiveInput, this.sanitizeFinalResponse(finalResponse), `meta-review=true; priorQuality=${reflectionQuality ?? "unknown"}; priorIssues=${reflectionIssueSnapshot.join(" | ")}`);
        ...
        // Add to memory
        await this.memory.addShortTerm(`User: ${userInput.slice(0, 100)} | Agent: ${finalResponse.slice(0, 100)}`, 2, this.conversation.id);
        return {
