...
function normalizeNodes(value) {
    ...
    for (let index = 0; index < value.length; index++) {
        ...
        const dependsOn = Array.isArray(node["dependsOn"])
            ? node["dependsOn"].map((dep) => String(dep)).filter((dep) => dep.length > 0)
            : [];
        nodes.push({
            id,
            title,
...
function validateNodesAndEdges(nodes, edges) {
    const ids = new Set();
    for (const node of nodes) {
        const id = String(node.id ?? "").trim();
        if (!id)
            return "workflow: nodes require non-empty id";
        if (ids.has(id))
            return `workflow: duplicate node id '${id}'`;
        ids.add(id);
    ...
    for (const edge of edges) {
        if (!ids.has(edge.source))
            return `workflow: edge source '${edge.source}' does not exist`;
        if (!ids.has(edge.target))
            return `workflow: edge target '${edge.target}' does not exist`;
    ...
    for (const [nodeId, deps] of dependencies.entries()) {
        for (const dep of deps) {
            if (!ids.has(dep))
                return `workflow: node '${nodeId}' depends on unknown node '${dep}'`;
        }
    }
    ...
    for (const nodeId of ids) {
        if (hasCycle(nodeId))
            return "workflow: cyclic dependencies detected";
    }
    return undefined;
}
export function createWorkflowManagementTool(workflowEngine) {
    return {
        name: "workflow",
            ...
            try {
                switch (action) {
                    ...
                    case "get": {
                        ...
                        if (!id)
                            return fail("workflow:get requires field 'id'");
                        const workflow = await workflowEngine.getWorkflow(id);
                        if (!workflow)
                            return fail(`Workflow '${id}' not found`);
                        return ok(workflow);
                    }
                    case "create": {
                        const name = String(input["name"] ?? "").trim();
                        if (!name)
                            return fail("workflow:create requires field 'name'");
                        ...
                        const status = ["draft", "running", "completed", "failed"].includes(statusRaw)
                            ? statusRaw
                            : "draft";
                    ...
                    case "update": {
                        const id = String(input["id"] ?? "").trim();
                        if (!id)
                            return fail("workflow:update requires field 'id'");
                    ...
                    case "run": {
                        const id = String(input["id"] ?? "").trim();
                        if (!id)
                            return fail("workflow:run requires field 'id'");
                        return ok(await workflowEngine.runWorkflow(id));
                    }
                    case "resume": {
                        const id = String(input["id"] ?? "").trim();
                        if (!id)
                            return fail("workflow:resume requires field 'id'");
                        return ok(await workflowEngine.resumeWorkflow(id));
                    }
                    case "delete": {
                        const id = String(input["id"] ?? "").trim();
                        if (!id)
                            return fail("workflow:delete requires field 'id'");
...
//# sourceMappingURL=workflow-management-tool.js.map