...
export class ThinkBlockParser {
    ...
    parse(content) {
        ...
        const statistics = {
            totalThinkTokens: thinkBlocks.reduce((sum, block) => sum + (block.tokenEstimate || 0), 0),
            totalToolRefs: thinkBlocks.reduce((sum, block) => sum + block.toolCalls.length, 0),
     ...
     */
    extractXmlBlocks(content, blocks, ranges) {
        const patterns = [
     ...
     */
    extractMarkdownBlocks(content, blocks, ranges) {
     ...
     */
    extractCustomBlocks(content, blocks, ranges) {
     ...
     * - "use tool X" (English)
     */
    extractToolReferences(thinkContent) {
        const references = [];
        ...
        for (const pattern of allPatterns) {
            ...
            while ((match = pattern.regex.exec(thinkContent)) !== null) {
                ...
                if (toolName.length > 0) {
                    ...
                    const isDuplicate = references.some((ref) => ref.toolName === toolName &&
     ...
     */
    determineThinkingDepth(content) {
...
//# sourceMappingURL=think-block-parser.js.map