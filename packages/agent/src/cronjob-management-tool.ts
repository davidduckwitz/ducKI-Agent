...
export function createCronjobManagementTool(db: DatabaseService): ToolExecutor {
  return {
    name: "cronjob",
      ...
      try {
        switch (action) {
          ...
          case "get": {
            ...
            if (!Number.isFinite(id) || id <= 0) return fail("cronjob:get requires numeric 'id'");
            const job = await db.getCronJob(id);
            if (!job) return fail(`Cronjob '${id}' not found`);
            return ok(job);
          }
          case "create": {
            ...
            if (!name) return fail("cronjob:create requires 'name'");
            if (!schedule) return fail("cronjob:create requires 'schedule'");
            if (!isCronExpressionValid(schedule)) return fail("cronjob:create received invalid cron expression");
            if (!targetType) return fail("cronjob:create requires valid 'targetType' (task|prompt|tool|skill)");
          ...
          case "update": {
            const id = Number(input["id"]);
            if (!Number.isFinite(id) || id <= 0) return fail("cronjob:update requires numeric 'id'");

            const patch: Record<string, unknown> = {};
            if (input["name"] !== undefined) patch["name"] = String(input["name"] ?? "").trim();
            if (input["schedule"] !== undefined) {
              const schedule = String(input["schedule"] ?? "").trim();
              if (!isCronExpressionValid(schedule)) return fail("cronjob:update received invalid cron expression");
              patch["schedule"] = schedule;
            }
            if (input["targetType"] !== undefined) {
              const targetType = asTargetType(input["targetType"]);
              if (!targetType) return fail("cronjob:update requires valid 'targetType' when provided");
            ...
            if (!updated) return fail(`Cronjob '${id}' not found`);
            return ok(updated);
          }
          case "delete": {
            const id = Number(input["id"]);
            if (!Number.isFinite(id) || id <= 0) return fail("cronjob:delete requires numeric 'id'");
