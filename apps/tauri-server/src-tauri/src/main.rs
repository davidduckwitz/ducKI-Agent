...
fn start_backend_server(app: &AppHandle) -> Result<(), String> {
    ...
    let home_dir = app.path().home_dir().map_err(|e| format!("Failed to resolve user home: {}", e))?;
    std::fs::create_dir_all(home_dir.join("DucKI").join("skills")).map_err(|e| e.to_string())?;
    seed_core_skills(app, &home_dir.join("DucKI").join("skills"))?;
    let plugins_dir = seed_builtin_plugins(app, &home_dir);
    ...
    let (mut rx, child) = app
        ...
        .env("SKILLS_PATH", home_dir.join("DucKI").join("skills").to_string_lossy().to_string())
        .spawn()
