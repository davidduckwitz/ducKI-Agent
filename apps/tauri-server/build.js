...
// Instead of copying apps/server/dist (~hundreds of compiled files) plus every @ducki/*
// workspace package's dist plus a full `npm install` of ~240 external packages, esbuild bundles
// the server entry point - and every @ducki/* workspace package it imports - into a single JS
// file, resolving pnpm's workspace symlinks at BUILD time (inside the monorepo, where they're
// valid) instead of relying on them surviving into the packaged app. Only packages that ship a
// native addon, spawn/locate a binary relative to their own package dir, or do dynamic
