...
Treat that as relative to the active working directory / backend workspace. DucKI file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.
